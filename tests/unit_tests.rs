...
#[test]
fn test_async_request_status_serialization() {
    use a_prox::async_queue::models::AsyncRequestStatus;
    ...
    for status in statuses {
        ...
        let deserialized: AsyncRequestStatus = serde_json::from_str(&json).unwrap();
        assert_eq!(status, deserialized);
    }
}

#[test]
fn test_async_request_new() {
    use a_prox::async_queue::models::AsyncRequest;
    ...
    assert_eq!(request.payload, r#"{"messages": []}"#);
    assert_eq!(request.status, a_prox::async_queue::models::AsyncRequestStatus::Queued);
    ...
    assert!(request.updated_at <= after);
    // TTL is 1 hour = 3600 seconds
    assert!(request.expires_at >= before + 3600);
    assert!(request.expires_at <= after + 3600);
    assert!(request.route_decision.is_none());
    assert_eq!(request.tokens_received, 0);
    assert!(request.error.is_none());
}

#[test]
fn test_async_request_is_expired() {
    use a_prox::async_queue::models::AsyncRequest;
    use std::time::{SystemTime, UNIX_EPOCH};
    ...
    let expired_request = AsyncRequest {
        id: "expired".to_string(),
        payload: "{}".to_string(),
        status: a_prox::async_queue::models::AsyncRequestStatus::Queued,
        result: None,
        created_at: now - 7200,
        updated_at: now - 7200,
        expires_at: now - 3600,
    ...
    let valid_request = AsyncRequest {
        id: "valid".to_string(),
        payload: "{}".to_string(),
        status: a_prox::async_queue::models::AsyncRequestStatus::Queued,
        result: None,
    ...
    assert!(!valid_request.is_expired());
}

#[test]
fn test_async_request_is_terminal() {
    use a_prox::async_queue::models::{AsyncRequest, AsyncRequestStatus};
    ...
    for status in terminal_statuses {
        ...
        assert!(req.is_terminal(), "Status {:?} should be terminal", status);
    }
    ...
    for status in non_terminal_statuses {
        ...
        assert!(!req.is_terminal(), "Status {:?} should not be terminal", status);
    }
}

#[test]
fn test_submit_async_request_deserialization() {
    use a_prox::async_queue::models::SubmitAsyncRequest;
    ...
    let request: SubmitAsyncRequest = serde_json::from_value(json).unwrap();
    assert_eq!(request.request_id, Some("custom-id-123".to_string()));
    assert!(request.payload.get("messages").is_some());
    assert_eq!(request.payload["stream"], true);
}

#[test]
fn test_submit_async_request_without_id() {
    use a_prox::async_queue::models::SubmitAsyncRequest;
...
#[test]
fn test_async_status_response_serialization() {
    use a_prox::async_queue::models::AsyncStatusResponse;
    
    let response = AsyncStatusResponse {
        request_id: "test-123".to_string(),
        status: "completed".to_string(),
        route_decision: Some("AgenticToolLoop".to_string()),
        tokens_received: 150,
        error: None,
        created_at: 1000,
        updated_at: 2000,
    ...
    assert_eq!(json["request_id"], "test-123");
    assert_eq!(json["status"], "completed");
    assert_eq!(json["route_decision"], "AgenticToolLoop");
    assert_eq!(json["tokens_received"], 150);
    assert_eq!(json["error"], serde_json::Value::Null);
    assert_eq!(json["created_at"], 1000);
    assert_eq!(json["updated_at"], 2000);
    assert_eq!(json["expires_at"], 3000);
}

#[test]
fn test_async_result_response_serialization() {
    use a_prox::async_queue::models::AsyncResultResponse;
    
    let response = AsyncResultResponse {
        request_id: "test-456".to_string(),
        status: "completed".to_string(),
        result: Some(json!({"choices": [{"message": {"content": "Hello"}}]})),
        route_decision: Some("FastPassThrough".to_string()),
        tokens_received: 50,
        error: None,
        created_at: 1000,
        completed_at: 2000,
    ...
    let json = serde_json::to_value(&response).unwrap();
    assert_eq!(json["request_id"], "test-456");
    assert_eq!(json["status"], "completed");
    assert!(json["result"].is_object());
    assert_eq!(json["route_decision"], "FastPassThrough");
    assert_eq!(json["tokens_received"], 50);
    assert_eq!(json["completed_at"], 2000);
}

#[tokio::test]
async fn test_async_request_repo_create_and_get() {
    use a_prox::async_queue::models::AsyncRequest;
    use a_prox::async_queue::worker::AsyncRequestRepo;
    use sqlx::SqlitePool;
    
    let temp_db = format!("sqlite:{}?mode=rwc", test_db_path("async_repo"));
    let pool = SqlitePool::connect(&temp_db).await.unwrap();
    
    // Run migrations
    sqlx::query(a_prox::db::schema::INIT_SQL).execute(&pool).await.unwrap();
    
    let repo = AsyncRequestRepo::new(pool.clone());
    
    let request = AsyncRequest::new(
        "test-repo-1".to_string(),
        r#"{"messages": [{"role": "user", "content": "test"}]}"#.to_string(),
        1,
    );
    
    // Create new request
    let created = repo.create(request.clone()).await.unwrap();
    assert_eq!(created.id, "test-repo-1");
    assert_eq!(created.payload, request.payload);
    
    // Get the request
    let fetched = repo.get("test-repo-1").await.unwrap().unwrap();
    assert_eq!(fetched.id, "test-repo-1");
    assert_eq!(fetched.payload, request.payload);
    assert_eq!(fetched.status, a_prox::async_queue::models::AsyncRequestStatus::Queued);
    
    // Try to create again (idempotent)
    let created_again = repo.create(request).await.unwrap();
    assert_eq!(created_again.id, "test-repo-1");
    
    // Cleanup
    drop(pool);
    cleanup_test_db(&temp_db.replace("sqlite:", "").replace("?mode=rwc", ""));
}

#[tokio::test]
async fn test_async_request_repo_update_status() {
    use a_prox::async_queue::models::{AsyncRequest, AsyncRequestStatus};
    use a_prox::async_queue::worker::AsyncRequestRepo;
    use sqlx::SqlitePool;
    
    let temp_db = format!("sqlite:{}?mode=rwc", test_db_path("async_repo_status"));
    let pool = SqlitePool::connect(&temp_db).await.unwrap();
    ...
    repo.update_status("test-status-1", AsyncRequestStatus::Processing).await.unwrap();
    let fetched = repo.get("test-status-1").await.unwrap().unwrap();
    assert_eq!(fetched.status, AsyncRequestStatus::Processing);
    
    // Update to completed
    repo.update_status("test-status-1", AsyncRequestStatus::Completed).await.unwrap();
    let fetched = repo.get("test-status-1").await.unwrap().unwrap();
    assert_eq!(fetched.status, AsyncRequestStatus::Completed);
...
#[tokio::test]
async fn test_async_request_repo_complete() {
    use a_prox::async_queue::models::{AsyncRequest, AsyncRequestStatus};
    use a_prox::async_queue::worker::AsyncRequestRepo;
    use sqlx::SqlitePool;
    
    let temp_db = format!("sqlite:{}?mode=rwc", test_db_path("async_repo_complete"));
    let pool = SqlitePool::connect(&temp_db).await.unwrap();
    ...
    repo.complete(
        "test-complete-1",
        AsyncRequestStatus::Completed,
        Some(r#"{"choices": [{"message": {"content": "Done"}}]}"#.to_string()),
        Some("AgenticToolLoop".to_string()),
        100,
        None,
    ).await.unwrap();
    
    let fetched = repo.get("test-complete-1").await.unwrap().unwrap();
    assert_eq!(fetched.status, AsyncRequestStatus::Completed);
    assert_eq!(fetched.result, Some(r#"{"choices": [{"message": {"content": "Done"}}]}"#.to_string()));
    assert_eq!(fetched.route_decision, Some("AgenticToolLoop".to_string()));
    assert_eq!(fetched.tokens_received, 100);
...
#[tokio::test]
async fn test_async_request_repo_cancel() {
    use a_prox::async_queue::models::{AsyncRequest, AsyncRequestStatus};
    use a_prox::async_queue::worker::AsyncRequestRepo;
    use sqlx::SqlitePool;
    
    let temp_db = format!("sqlite:{}?mode=rwc", test_db_path("async_repo_cancel"));
    let pool = SqlitePool::connect(&temp_db).await.unwrap();
    ...
    // Cancel the request
    repo.cancel("test-cancel-1").await.unwrap();
    
    let fetched = repo.get("test-cancel-1").await.unwrap().unwrap();
    assert_eq!(fetched.status, AsyncRequestStatus::Cancelled);
...
#[tokio::test]
async fn test_async_queue_worker_basic() {
    use a_prox::async_queue::worker::AsyncQueueWorker;
    use a_prox::state::AppState;
    use std::sync::Arc;
    use tokio::sync::Semaphore;
    
    // This test verifies the worker can be created with proper config
    // We don't run the full worker loop as it requires a full AppState
    
    // Just verify the module compiles and types are correct
    let _worker_type = std::any::type_name::<AsyncQueueWorker>();
    let _semaphore_type = std::any::type_name::<Semaphore>();
    let _arc_type = std::any::type_name::<Arc<AppState>>();
}

#[test]
fn test_async_config_default() {
    use a_prox::config::AsyncConfig;
