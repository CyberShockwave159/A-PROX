...
- Phase C: synthetic user message (text + base64 data-URL of the PNG) → llama streams caption; ONE `delta.image_url` SSE event emitted before text (`image_url_sse_payload`); non-streaming: `image_url` on outer JSON. The URL uses the per-request `artifact_base_url` resolution (or a base64 data-URL when `[image_generation].inline_data_url = true`) — see the resolution bullet in File Write.
