...
- Config: `[file_generation]` `enabled`, `serve_dir`, `public_base_url` (empty → `http://127.0.0.1:{server.port}`), `max_content_chars`, `deny_exts`.
...
## Testing notes
- Main suite is `tests/unit_tests.rs`; the only inline `#[cfg(test)]` modules under `src/` are `src/router/intent.rs`, `src/imagegen/{ratio.rs,tests.rs}`, `src/filegen/mod.rs`, `src/files/store.rs`, and `src/server/routes.rs` (`filegen_tests` for `sanitize_tool_calls_for_history`).
